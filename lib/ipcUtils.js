...
var registerIpcListeners = function() {
    ...
    ipc.on('add-urlacl-entry', function(event, data) {
        ...
        try {
            ...
            console.log('Error delete urlacl: ' + e.stderr);
        }
        ...
        try {
            ...
            console.log('Error add urlacl: ' + e.stderr);
        }

        event.returnValue = "UrlAcl entry added successfully";
    });

    ipc.on('add-firewall-entry', function(event, port) {
        let deleteCommand = 'netsh advfirewall firewall delete rule name=IISExpressWeb';
        try {
            execSync(deleteCommand);
        }
        catch (e) {
            console.log('Error delete firewall rule: ' + e.stderr);
        }
        ...
        try {
            ...
            console.log('Error add firewall rule: ' + e.stderr);
        }
